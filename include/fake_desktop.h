...
namespace gd
{
	...
	public:
		...
		void init(gd::Graph& graph)
		{
			...
			if (result.isErr || result.isNone)
			{
				Windows::error(result.description);
				closeWindow();
			}

			// fakeDesktopWindowの親ウィンドウを壁紙のウィンドウに設定
			SetParent(hWnd, result.value.wallpaper);

			// ウィンドウの位置とサイズを調整する
			RECT rc;
			GetWindowRect(result.value.wallpaper, &rc);
			SetWindowPos(hWnd, HWND_BOTTOM, 0, 0, rc.right - rc.left, rc.bottom - rc.top, 0);
			setSize(rc.right - rc.left, rc.bottom - rc.top);

			// デスクトップの祖先ウィンドウを透明にする
			// (デスクトップは不透明のままにする)
	...
	private:
		...
		// デスクトップのウィンドウハンドルを取得する
		Result<DesktopHwnds> getDesktopHwnd()
		{
			DesktopHwnds result;
	...
	class FakeDesktopComponent2 : public RootComponent
	{
	public:
		struct DesktopHwnds
		{
		...
		FakeDesktopComponent2() {}
		virtual ~FakeDesktopComponent2() {}
		...
		void init(gd::Graph& graph)
		{
			...
			// キャプチャを開始する
			bool capRet = false;
			capRet |= listview.start(graph.getDeviceContext(), result.value.listview);
			capRet |= wallpaper.start(graph.getDeviceContext(), result.value.wallpaper);
			if (capRet)
			{
				Windows::error(L"デスクトップのキャプチャに失敗しました。");
				closeWindow();
			}
		}
		void render(gd::Graph& graph, gd::Mouse& mouse) override
		{
			...
			graph.image(wallpaper.getBackupImage());
			graph.setRenderMode(BlendMode::AlphaBlend, DepthMode::DepthNone, RasterizerMode::CullNone);
			graph.image(listview.getImage());
		}
		void exit(gd::Graph& graph) override
		{
			RootComponent::exit(graph);
